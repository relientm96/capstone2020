...
'''
pretrained model and Rolling Window Data Structure
'''
saved_model = 'final_lstm_activity_iter_03.h5'

import RollingWindow as RW
# Note numb joints here means both x,y values, (eg: if BODY_25 we have 25*2 numb joints)
numbJoints   = 36       # COCO model; only the body;
window_Width = 32       # activity recognition;
...
def loadModel():
    global lstm
    try:
        lstm = keras.models.load_model(saved_model, compile=False)
        lstm.summary()
    except Exception as e:
...
def removeConfidenceNumpy(datum):
    ...
    keypoints = datum.poseKeypoints[0][:,0:2]
