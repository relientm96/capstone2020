...
# modified/abstracted (on-hold) /commented by - nebulaM78; capstone 2020;
...
'''
DATASET_PATH = "C:\\Users\\yongw4\\Desktop\\yick\\lstm-tutorial\\dataset\\"
X_train_path =  "C:\\Users\\yongw4\\Desktop\\X_train.txt"
X_test_path =  "C:\\Users\\yongw4\\Desktop\\X_test.txt"

y_train_path = "C:\\Users\\yongw4\\Desktop\\Y_train.txt"
y_test_path = 'C:\\Users\\yongw4\\Desktop\\Y_test.txt'
'''
DATASET_PATH = "C:\\Users\\yongw4\\Desktop\\TREASURE\\"
X_train_path =  DATASET_PATH + "X_train.txt"
X_test_path =   DATASET_PATH + "X_test.txt"
...
# "rolling window"; one could view this as a shift register for the input;
...
training_iters = training_data_count*500  # Loop 300 times on the dataset, ie 300 epochs
#batch_size = 512
#batch_size = 32
batch_size = 75
...
if RETRAIN:		
	# the meta file if saved, will be at the current directory;
	# assert that the saved tf meta files are there before resuming the training;
	if (RESUME_TRAINING and os.path.isfile('./saved_final_model.meta')):
		print('restoring the saved trained model to resume training\n')
		tf.reset_default_graph()
		imported_meta = tf.train.import_meta_graph("saved_final_model.meta")
		imported_meta.restore(sess, tf.train.latest_checkpoint('./'))
		print("Model restored; ready to resume the training\n.")
	...
	while TRAINING_STEPS <= training_iters:
		...
		batch_xs, raw_labels, unsampled_indices = lstm.extract_batch_size(X_train, y_train, unsampled_indices, batch_size)
...
# note - the test sets are not needed for the learning here
#  - just evaluation for diagnosis
#---------------------------------------------------------------------------------------------
...
#-----------------------------------------------------------------------
# offline prediction with your own recorded video;
#-----------------------------------------------------------------------
print('offline prediction\n')
X_val_path = "C:\\Users\\yongw4\\Videos\\basic\\testing\\pain_dummy01_dummy02.txt"
...
# numpy array of the form ONEHOT = [x1,x2];
offline_onehot_pred = preds[0]

# note - offline_onehot_pred.argmax(1) = [int(max(ONEHOT))], a numpy array of one element only;
offline_pred = (offline_onehot_pred.argmax(1))
print('one-hot vectors prediction\n', offline_onehot_pred) 
...
for i in range(0, len(offline_pred)):
	sign = MAP_DICT[offline_pred[i]]
